...
PortBridge is a Go + Fyne desktop app for managing SSH port-forward tunnels. `cmd/port-bridge` contains the entry point, tray setup, platform-specific startup files, embedded translations in `cmd/port-bridge/translations/`, and application icon assets in `cmd/port-bridge/assets/`. Core code lives in `internal/`: `app` wires services together, `models` defines persisted types, `ssh` manages clients and tunnel lifecycle, `storage` saves JSON config, `ui` and `presenter` handle Fyne screens and callbacks, and `updater`, `i18n`, `logger`, `secure`, and `version` provide supporting services. The Go module path is `github.com/young1lin/port-bridge`, but runtime directory and binary are named `port-bridge`. Release helpers live in `scripts/`; generated binaries go to `build/`.

## Build, Test, and Development Commands
Use the Makefile as the default workflow. `make deps` downloads and tidies modules. `make fmt` runs `go fmt ./...`. `make run` launches the app in development mode. `make build` produces `build/port-bridge.exe` with `windowsgui`, version metadata, and `-trimpath`; `make build-debug` keeps the console window for logs. `make test-unit` runs the pure unit suite, `make test-gui` runs the Fyne/CGO GUI suite, `make test-all` runs unit + GUI + SSH integration, and `make test-coverage` enforces the core-package coverage gate.
